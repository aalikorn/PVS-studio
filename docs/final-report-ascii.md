...
- ? Code path leads to security violation
- ? Repercussions affect confidentiality/integrity/availability
- ? Exploitable via PoC script
- ? Unresolved in VULN mode, fixed in FIXED mode
...
| --- | ------ | --- | -------- | ------------------- | ------- | ------------------------------------------------------------------------------------------- | ------------------ | ------- |
| 1   | V6088  | 639 | CRITICAL | ApiController.java  | 154-176 | No authorization check in getUserDocs() ? any authenticated user can access any user's docs | Real Vulnerability | ? Fixed |
| 2   | V6089  | 347 | CRITICAL | JwtUtil.java        | 96-100  | Unsecured JWT parsing ? accepts alg=none tokens without signature verification              | Real Vulnerability | ? Fixed |
| 3   | V6090  | 521 | CRITICAL | JwtUtil.java        | 54-56   | Weak JWT secret "weak_secret_123" (18 chars) ? brute-forceable                              | Real Vulnerability | ? Fixed |
| 4   | V1234  | 200 | HIGH     | ApiController.java  | 164-174 | Token payload exposed in response ? helps attackers understand token structure              | Real Vulnerability | ? Fixed |
...
return Jwts.parser()
        .verifyWith(key)  // ? FIXED: Strict signature verification
        .build()
...
- ? Verified reproducible with PoC scripts
- ? Different behavior in VULN vs FIXED confirms intentional vulnerability
- ? No null-check or safety wrapping that could justify warnings
- ? Direct impact on CIA triad demonstrated
...
- ? IDOR patterns (custom rule) ? Found absence of ownership check
- ? Weak JWT parsing ? Flagged `unsecured()` call
- ? Weak secret detection ? Identified short key length

**What PVS-Studio Missed:**

- ? Logic-level authorization flaws (would need semantic analysis beyond SAST)
- ? Business logic errors (not in tool scope)
- ? Runtime behavior (static analysis limitation)
...
- ? 3 real vulnerabilities with CVSS scores ? 7.5 (all critical/high)
- ? All vulnerabilities reproducible with provided exploit scripts
- ? All vulnerabilities eliminated in FIXED mode
- ? 37.5% false positive rate in automated analysis
- ? Custom PVS-Studio rules effectively detect security-specific patterns
...
**Remediation Status:** ? **COMPLETE** ? All vulnerabilities fixed in FIXED mode
...
| ------------------------ | ------------------- | -------------------------- |
| **Authorization**        | ? No checks         | ? Enforced                 |
| **JWT Validation**       | ? Skipped           | ? Strict                   |
| **Cryptographic Key**    | ? Weak (18 chars)   | ? Strong (32+ chars)       |
| **Information Exposure** | ? Token in response | ? Removed                  |
| **Vulnerability Count**  | 3 Critical          | 0 Critical                 |
| **OWASP Score**          | ? Fails             | ? Passes A01, A02 controls |
